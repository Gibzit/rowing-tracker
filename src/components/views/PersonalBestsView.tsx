...
const CATEGORY_COLORS: Record<WorkoutCategory, { bg: string; text: string; icon: string }> = {
  distance: { bg: 'bg-[#B8941F]/10', text: 'text-[#B8941F]', icon: '#B8941F' },
  interval: { bg: 'bg-[#2E86AB]/10', text: 'text-[#2E86AB]', icon: '#2E86AB' },
  time: { bg: 'bg-[#45A868]/10', text: 'text-[#45A868]', icon: '#45A868' },
};

function CategoryIcon({ category, className }: { category: WorkoutCategory; className?: string }) {
  const color = CATEGORY_COLORS[category].icon;
...
export default function PersonalBestsView({ sessions, plan, onGoToTraining }: PersonalBestsViewProps) {
  ...
  return (
    <div className="py-6 px-5">
      ...
      <p className="text-xs text-gray-400 dark:text-[#5a6580] mb-6">
        Fastest pace by workout type
      </p>
      ...
      {pbs.length === 0 ? (
        ...
        <div className="space-y-4">
          {pbs.map((pb) => {
            const colors = CATEGORY_COLORS[pb.category];
            return (
              <div
                key={pb.category}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg ${colors.bg} ${colors.text}`}>
                        {pb.categoryLabel}
                      </span>
                      {pb.sessionCount > 1 && (
                        <span className="text-[10px] font-bold font-mono bg-gray-100 dark:bg-[#1a2640] text-gray-500 dark:text-gray-400 px-2 py-1 rounded-lg shrink-0">
                          {pb.sessionCount} sessions
                        </span>
                      )}
                    </div>
                    <p className="text-4xl font-mono font-bold text-gray-900 dark:text-[#dae2fd] tracking-tight">
                      {pb.paceFormatted}
                    ...
                    <p className="text-xs text-gray-400 dark:text-[#5a6580] mt-1">
                      Set in: {pb.workoutLabel}
                    </p>
                  </div>

                  <div className={`w-12 h-12 rounded-xl ${colors.bg} flex items-center justify-center shrink-0`}>
                    <CategoryIcon category={pb.category} className="w-6 h-6" />
                  </div>
                </div>

                {/* Improvement indicator */}
                {pb.improvementPct !== undefined && pb.sessionCount > 1 && pb.improvementPct < 0 && (
                  <div className="flex items-center gap-1.5 mt-3 text-green-600 dark:text-green-400">
                    <svg className="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M5.293 9.707a1 1 0 010-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414L11 7.414V15a1 1 0 11-2 0V7.414L6.707 9.707a1 1 0 01-1.414 0z" clipRule="evenodd" />
                    </svg>
                    <span className="text-xs font-mono font-bold">
                      {Math.abs(pb.improvementPct)}% faster than avg
                    </span>
                  </div>
                )}
